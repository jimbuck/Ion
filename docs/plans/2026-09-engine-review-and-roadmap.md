...
- `Ion.Shaders` and `Ion.Shaders.targets`: build-time GLSL 4.5 to SPIR-V (Shaderc) and GLSL ES 3.10 (SPIRV-Cross, combined image samplers, depth fixup), embedded as `Shaders/<file>.spv` and `.es.glsl`. Not done: reflection-generated C# layouts and the flattening of (set, binding) to GLES binding points, which the GLES backend needs.
