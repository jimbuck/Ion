...
| `UiBenchmarks` | CPU cost of a 500-widget UI frame (Ion.Extensions.UI): building, flex layout and tree publication, and the same plus the draw submission into the 2D renderer's `SpriteBatch` |
| `WebBenchmarks` | The web module on loopback with the web step run continuously: a keep-alive `GET` round trip (parse, route, game-thread handler, JSON number, response) and WebSocket push throughput to 1 and 4 clients (1,000 broadcasts per operation; the allocation column counts the benchmark's own `ClientWebSocket` receivers) |
