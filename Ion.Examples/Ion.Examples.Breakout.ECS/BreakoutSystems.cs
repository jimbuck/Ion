...
// The Breakout ECS game's components, events and systems, shared by the desktop program, the tests and the mobile heads.
...
public class PaddleSystem(IWindow window, World world, IInputState input, IEvents events, IAssetManager assets)
{
	...
	/// <summary>
	/// Touch screens (the Android and iOS heads): the paddle follows the first finger, and lifting it launches a ball.
	/// Touches have a per-frame view only, so they are read here in Update rather than in the fixed step.
	/// </summary>
	[Update]
